    ...
    match record {
        ...
        CwrRegistry::Swr(mut swr) => {
            ...
            if let Some(ref ipi_name) = swr.writer_ipi_name_num {
                swr.writer_ipi_name_num = Some(allegro_cwr::domain_types::IpiNameNumber(mappings.obfuscate_ipi(ipi_name)));
            }
            if let Some(ref ipi_base) = swr.writer_ipi_base_number {
                swr.writer_ipi_base_number = Some(allegro_cwr::domain_types::IpiBaseNumber(mappings.obfuscate_ipi(ipi_base)));
            }
            CwrRegistry::Swr(swr)
        }
        CwrRegistry::Spu(mut spu) => {
            ...
            if let Some(ref ipi_name) = spu.publisher_ipi_name_num {
                spu.publisher_ipi_name_num = Some(allegro_cwr::domain_types::IpiNameNumber(mappings.obfuscate_ipi(ipi_name)));
            }
            if let Some(ref ipi_base) = spu.publisher_ipi_base_number {
                spu.publisher_ipi_base_number = Some(allegro_cwr::domain_types::IpiBaseNumber(mappings.obfuscate_ipi(ipi_base)));
            }
            CwrRegistry::Spu(spu)
        }
        ...
        CwrRegistry::Per(mut per) => {
            ...
            if let Some(ref ipi_name) = per.performing_artist_ipi_name_num {
                per.performing_artist_ipi_name_num = Some(allegro_cwr::domain_types::IpiNameNumber(mappings.obfuscate_ipi(ipi_name)));
            }
            if let Some(ref ipi_base) = per.performing_artist_ipi_base_number {
                per.performing_artist_ipi_base_number = Some(allegro_cwr::domain_types::IpiBaseNumber(mappings.obfuscate_ipi(ipi_base)));
            }
            CwrRegistry::Per(per)
        }
        ...
        CwrRegistry::Com(mut com) => {
            ...
            if let Some(ref ipi_name) = com.writer_1_ipi_name_num {
                com.writer_1_ipi_name_num = Some(allegro_cwr::domain_types::IpiNameNumber(mappings.obfuscate_ipi(ipi_name)));
            }
            if let Some(ref ipi_name) = com.writer_2_ipi_name_num {
                com.writer_2_ipi_name_num = Some(allegro_cwr::domain_types::IpiNameNumber(mappings.obfuscate_ipi(ipi_name)));
            }
            if let Some(ref ipi_base) = com.writer_1_ipi_base_number {
                com.writer_1_ipi_base_number = Some(allegro_cwr::domain_types::IpiBaseNumber(mappings.obfuscate_ipi(ipi_base)));
            }
            if let Some(ref ipi_base) = com.writer_2_ipi_base_number {
                com.writer_2_ipi_base_number = Some(allegro_cwr::domain_types::IpiBaseNumber(mappings.obfuscate_ipi(ipi_base)));
            }
            CwrRegistry::Com(com)
        }
