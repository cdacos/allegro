...
/// This eliminates the dangerous get_mandatory_field! macro that corrupts data
fn handle_record_with_struct<T, F>(
    line_number: usize,
    tx: &Transaction,
    stmts: &mut PreparedStatements,
    context: &ParsingContext,
    safe_slice: &impl Fn(usize, usize) -> Result<Option<String>, CwrParseError>,
    parse_fn: fn(&str) -> Result<T, CwrParseError>,
    insert_fn: F,
...
pub fn parse_and_insert_hdr<'a>(
    ...
    handle_record_with_struct(
        line_number,
        tx,
        stmts,
        context,
        safe_slice,
        HdrRecord::from_cwr_line,
        |record, stmts, file_id| {
...
pub fn parse_and_insert_trl<'a>(
    ...
    handle_record_with_struct(
        ...
        context,
        safe_slice,
        TrlRecord::from_cwr_line,
        |record, stmts, file_id| {
            stmts.trl_stmt.execute(params![
                file_id,
                record.record_type,
                record.group_count,
                record.transaction_count,
...
pub fn parse_and_insert_alt<'a>(
    ...
    handle_record_with_struct(
        ...
        context,
        safe_slice,
        AltRecord::from_cwr_line,
        |record, stmts, file_id| {
            stmts.alt_stmt.execute(params![
                file_id,
                record.record_type,
                record.transaction_sequence_num,
                record.record_sequence_num,
                record.alternate_title,
                record.title_type,
...
pub fn parse_and_insert_ter<'a>(line_number: usize, tx: &'a Transaction, stmts: &'a mut PreparedStatements, context: &ParsingContext, safe_slice: &impl Fn(usize, usize) -> Result<Option<String>, CwrParseError>) -> Result<(), CwrParseError> {
    handle_record_with_struct(
        ...
        context,
        safe_slice,
        TerRecord::from_cwr_line,
        |record, stmts, file_id| {
            stmts.ter_stmt.execute(params![
                file_id,
                record.record_type,
                record.transaction_sequence_num,
                record.record_sequence_num,
                record.inclusion_exclusion_indicator,
                record.tis_numeric_code,
            ])?;
            Ok(())
        },
    )
    ...
    handle_record_with_struct(
        ...
        |record, stmts, file_id| {
            stmts.npa_stmt.execute(params![
                ...
                record.interested_party_writer_first_name.as_str(),
                record.language_code.as_deref().unwrap_or(""),
            ])?;
            Ok(())
        },
...
pub fn parse_and_insert_npn<'a>(line_number: usize, tx: &'a Transaction, stmts: &'a mut PreparedStatements, context: &ParsingContext, safe_slice: &impl Fn(usize, usize) -> Result<Option<String>, CwrParseError>) -> Result<(), CwrParseError> {
    handle_record_with_struct(
        ...
        context,
        safe_slice,
        NpnRecord::from_cwr_line,
        |record, stmts, file_id| {
            stmts.npn_stmt.execute(params![
                file_id,
                record.record_type,
                record.transaction_sequence_num,
                record.record_sequence_num,
...
pub fn parse_and_insert_nat<'a>(line_number: usize, tx: &'a Transaction, stmts: &'a mut PreparedStatements, context: &ParsingContext, safe_slice: &impl Fn(usize, usize) -> Result<Option<String>, CwrParseError>) -> Result<(), CwrParseError> {
    handle_record_with_struct(
        ...
        context,
        safe_slice,
        NatRecord::from_cwr_line,
        |record, stmts, file_id| {
            stmts.nat_stmt.execute(params![
                file_id,
                record.record_type,
                record.transaction_sequence_num,
                record.record_sequence_num,
...
pub fn parse_and_insert_ind<'a>(line_number: usize, tx: &'a Transaction, stmts: &'a mut PreparedStatements, context: &ParsingContext, safe_slice: &impl Fn(usize, usize) -> Result<Option<String>, CwrParseError>) -> Result<(), CwrParseError> {
    handle_record_with_struct(
        ...
        context,
        safe_slice,
        IndRecord::from_cwr_line,
        |record, stmts, file_id| {
            stmts.ind_stmt.execute(params![
                file_id,
                record.record_type,
                record.transaction_sequence_num,
                record.record_sequence_num,
                record.instrument_code,
                record.number_of_players.as_deref().unwrap_or(""),
            ])?;
            Ok(())
        },
    )
    ...
    handle_record_with_struct(
        ...
        |record, stmts, file_id| {
            stmts.msg_stmt.execute(params![
                ...
                record.record_sequence_num,
                record.message_type,
                record.original_record_sequence_num.as_str(),
                record.record_type_field.as_str(),
                record.message_level,
                record.validation_number.as_str(),
                record.message_text,
            ])?;
            Ok(())
        },
...
pub fn parse_and_insert_net<'a>(line_number: usize, tx: &'a Transaction, stmts: &'a mut PreparedStatements, context: &ParsingContext, safe_slice: &impl Fn(usize, usize) -> Result<Option<String>, CwrParseError>) -> Result<(), CwrParseError> {
    handle_record_with_struct(
        ...
        context,
        safe_slice,
        NetRecord::from_cwr_line,
        |record, stmts, file_id| {
            stmts.net_stmt.execute(params![
                file_id,
                record.record_type,
                record.transaction_sequence_num,
                record.record_sequence_num,
...
pub fn parse_and_insert_now<'a>(line_number: usize, tx: &'a Transaction, stmts: &'a mut PreparedStatements, context: &ParsingContext, safe_slice: &impl Fn(usize, usize) -> Result<Option<String>, CwrParseError>) -> Result<(), CwrParseError> {
    handle_record_with_struct(
        ...
        context,
        safe_slice,
        NowRecord::from_cwr_line,
        |record, stmts, file_id| {
            stmts.now_stmt.execute(params![
                file_id,
                record.record_type,
                record.transaction_sequence_num,
                record.record_sequence_num,
                record.writer_name,
                record.writer_first_name.as_str(),
                record.language_code.as_deref().unwrap_or(""),
...
pub fn parse_and_insert_xrf<'a>(line_number: usize, tx: &'a Transaction, stmts: &'a mut PreparedStatements, context: &ParsingContext, safe_slice: &impl Fn(usize, usize) -> Result<Option<String>, CwrParseError>) -> Result<(), CwrParseError> {
    handle_record_with_struct(
        ...
        context,
        safe_slice,
        XrfRecord::from_cwr_line,
        |record, stmts, file_id| {
            stmts.xrf_stmt.execute(params![
                file_id,
                record.record_type,
                record.transaction_sequence_num,
                record.record_sequence_num,
                record.organisation_code,
                record.identifier,
                record.identifier_type.as_str(),
                record.validity.as_str(),
            ])?;
            Ok(())
        },
    )
