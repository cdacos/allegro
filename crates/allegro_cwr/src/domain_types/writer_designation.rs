...
impl CwrFieldParse for WriterDesignation {
    fn parse_cwr_field(source: &str, field_name: &'static str, field_title: &'static str) -> (Self, Vec<CwrWarning<'static>>) {
        ...
        if !is_valid_writer_designation(&trimmed) {
            warnings.push(CwrWarning { field_name, field_title, source_str: Cow::Owned(source.to_string()), level: WarningLevel::Warning, description: format!("Writer Designation '{}' not found in lookup table", trimmed) });
        }
