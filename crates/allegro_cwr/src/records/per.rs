...
impl PerRecord {
    ...
    pub fn new(
        ...
        performing_artist_last_name: String,
    ) -> Self {
        Self {
            record_type: "PER".to_string(),
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let performing_artist_first_name = if line.len() > 64 {
            line.get(64..94)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let performing_artist_ipi_name_num = if line.len() > 94 {
            line.get(94..105)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let performing_artist_ipi_base_number = if line.len() > 105 {
            line.get(105..118)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
    ...
    pub fn to_cwr_line(&self) -> String {
        let mut fields = vec![
            ...
            format!("{:45}", self.performing_artist_last_name)
        ];
        
        if self.performing_artist_first_name.is_some() || 
           self.performing_artist_ipi_name_num.is_some() || 
           self.performing_artist_ipi_base_number.is_some() {
            fields.push(format!("{:30}", self.performing_artist_first_name.as_deref().unwrap_or("")));
        }
        
        if self.performing_artist_ipi_name_num.is_some() || 
           self.performing_artist_ipi_base_number.is_some() {
