...
impl OrnRecord {
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let production_title = if line.len() > 22 {
            line.get(22..82)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let cd_identifier = if line.len() > 82 {
            line.get(82..97)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let production_num = if line.len() > 187 {
            line.get(187..199)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let episode_title = if line.len() > 199 {
            line.get(199..259)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let episode_num = if line.len() > 259 {
            line.get(259..279)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let year_of_production = if line.len() > 279 {
            line.get(279..283)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let avi_society_code = if line.len() > 283 {
            line.get(283..286)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let audio_visual_number = if line.len() > 286 {
            line.get(286..301)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        // v2.2+ fields
        let v_isan_isan = if line.len() > 301 {
            line.get(301..313)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let v_isan_episode = if line.len() > 313 {
            line.get(313..317)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let v_isan_check_digit_1 = if line.len() > 317 {
            line.get(317..318)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let v_isan_version = if line.len() > 318 {
            line.get(318..326)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let v_isan_check_digit_2 = if line.len() > 326 {
            line.get(326..327)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let eidr_check_digit = if line.len() > 347 {
            line.get(347..348)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
    ...
    pub fn to_cwr_line(&self) -> String {
        let mut fields = vec![
            ...
            format!("{:3}", self.intended_purpose)
        ];
        
        if self.production_title.is_some() || self.cd_identifier.is_some() || self.cut_number.is_some() ||
           self.library.is_some() || self.bltvr.is_some() || self.filler.is_some() ||
           self.production_num.is_some() || self.episode_title.is_some() || self.episode_num.is_some() ||
           self.year_of_production.is_some() || self.avi_society_code.is_some() ||
           self.audio_visual_number.is_some() || self.v_isan_isan.is_some() || self.v_isan_episode.is_some() ||
           self.v_isan_check_digit_1.is_some() || self.v_isan_version.is_some() ||
           self.v_isan_check_digit_2.is_some() || self.eidr.is_some() || self.eidr_check_digit.is_some() {
            ...
            // v2.1+ fields
            if self.library.is_some() || self.bltvr.is_some() || self.filler.is_some() ||
               self.production_num.is_some() || self.episode_title.is_some() || self.episode_num.is_some() ||
               self.year_of_production.is_some() || self.avi_society_code.is_some() ||
               self.audio_visual_number.is_some() || self.v_isan_isan.is_some() || self.v_isan_episode.is_some() ||
               self.v_isan_check_digit_1.is_some() || self.v_isan_version.is_some() ||
               self.v_isan_check_digit_2.is_some() || self.eidr.is_some() || self.eidr_check_digit.is_some() {
                ...
                // v2.2+ fields
                if self.v_isan_isan.is_some() || self.v_isan_episode.is_some() ||
                   self.v_isan_check_digit_1.is_some() || self.v_isan_version.is_some() ||
                   self.v_isan_check_digit_2.is_some() || self.eidr.is_some() || self.eidr_check_digit.is_some() {
