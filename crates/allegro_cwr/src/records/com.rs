...
impl ComRecord {
    ...
    pub fn new(transaction_sequence_num: String, record_sequence_num: String, title: String, writer_1_last_name: String) -> Self {
        Self {
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let iswc_of_component = line.get(79..90).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string());

        let submitter_work_num = line.get(90..104).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string());

        let duration = line.get(104..110).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string());

        let writer_1_last_name = line.get(110..155).unwrap().trim().to_string();

        let writer_1_first_name = if line.len() > 155 { line.get(155..185).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let writer_1_ipi_name_num = if line.len() > 185 { line.get(185..196).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let writer_2_last_name = if line.len() > 196 { line.get(196..241).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let writer_2_first_name = if line.len() > 241 { line.get(241..271).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let writer_2_ipi_name_num = if line.len() > 271 { line.get(271..282).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let writer_1_ipi_base_number = if line.len() > 282 { line.get(282..295).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let writer_2_ipi_base_number = if line.len() > 295 { line.get(295..308).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        Ok(ComRecord { record_type, transaction_sequence_num, record_sequence_num, title, iswc_of_component, submitter_work_num, duration, writer_1_last_name, writer_1_first_name, writer_1_ipi_name_num, writer_2_last_name, writer_2_first_name, writer_2_ipi_name_num, writer_1_ipi_base_number, writer_2_ipi_base_number })
    }
    ...
    pub fn to_cwr_line(&self) -> String {
        ...
        if self.writer_1_first_name.is_some() || self.writer_1_ipi_name_num.is_some() || self.writer_2_last_name.is_some() || self.writer_2_first_name.is_some() || self.writer_2_ipi_name_num.is_some() || self.writer_1_ipi_base_number.is_some() || self.writer_2_ipi_base_number.is_some() {
...
mod tests {
    ...
    fn test_com_creation() {
        let com = ComRecord::new("00000001".to_string(), "00000001".to_string(), "Component Title".to_string(), "COMPOSER".to_string());
    ...
    fn test_com_round_trip() {
        let original = ComRecord::new("00000001".to_string(), "00000001".to_string(), "Component Title".to_string(), "COMPOSER".to_string());
