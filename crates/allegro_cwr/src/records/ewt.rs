...
impl EwtRecord {
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let iswc_of_entire_work = if line.len() > 79 {
            line.get(79..90)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
    ...
    pub fn to_cwr_line(&self) -> String {
        let mut fields = vec![
            ...
            format!("{:60}", self.entire_work_title)
        ];
        
        if self.iswc_of_entire_work.is_some() || self.language_code.is_some() ||
...
mod tests {
    ...
    fn test_ewt_creation() {
        let ewt = EwtRecord::new(
            "00000001".to_string(),
            "00000001".to_string(),
            "Entire Work Title".to_string(),
        );
