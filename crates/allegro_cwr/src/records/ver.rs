...
impl VerRecord {
    ...
    pub fn new(transaction_sequence_num: String, record_sequence_num: String, original_work_title: String) -> Self {
        Self {
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let iswc_of_original_work = if line.len() > 79 { line.get(79..90).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };
    ...
    pub fn to_cwr_line(&self) -> String {
        let mut fields = vec![format!("{:3}", self.record_type), format!("{:8}", self.transaction_sequence_num), format!("{:8}", self.record_sequence_num), format!("{:60}", self.original_work_title)];

        if self.iswc_of_original_work.is_some()
...
mod tests {
    ...
    fn test_ver_creation() {
        let ver = VerRecord::new("00000001".to_string(), "00000001".to_string(), "Original Work Title".to_string());
    ...
    fn test_ver_round_trip() {
        let original = VerRecord::new("00000001".to_string(), "00000001".to_string(), "Original Work Title".to_string());
