...
impl VerRecord {
    ...
    pub fn new(
        ...
        Self {
            record_type: "VER".to_string(),
            transaction_sequence_num,
            record_sequence_num,
            original_work_title,
            iswc_of_original_work: None,
            language_code: None,
    ...
    /// Parse a CWR line into a VER record
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        if line.len() < 79 {
            return Err(CwrParseError::BadFormat("VER line too short".to_string()));
        }
        ...
        let original_work_title = line.get(19..79).unwrap().trim().to_string();
        
        let iswc_of_original_work = if line.len() > 79 {
            line.get(79..90)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let language_code = if line.len() > 90 {
            line.get(90..92)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let writer_1_last_name = if line.len() > 92 {
            line.get(92..137)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let writer_1_first_name = if line.len() > 137 {
            line.get(137..167)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let writer_1_ipi_name_num = if line.len() > 227 {
            line.get(227..238)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let writer_1_ipi_base_number = if line.len() > 238 {
            line.get(238..251)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let writer_2_last_name = if line.len() > 251 {
            line.get(251..296)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let writer_2_first_name = if line.len() > 296 {
            line.get(296..326)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let writer_2_ipi_name_num = if line.len() > 326 {
            line.get(326..337)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let writer_2_ipi_base_number = if line.len() > 337 {
            line.get(337..350)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let submitter_work_num = if line.len() > 350 {
            line.get(350..364)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
    ...
    pub fn to_cwr_line(&self) -> String {
        let mut fields = vec![
            ...
            format!("{:60}", self.original_work_title)
        ];
        
        if self.iswc_of_original_work.is_some() || self.language_code.is_some() ||
           self.writer_1_last_name.is_some() || self.writer_1_first_name.is_some() ||
           self.source.is_some() || self.writer_1_ipi_name_num.is_some() ||
           self.writer_1_ipi_base_number.is_some() || self.writer_2_last_name.is_some() ||
           self.writer_2_first_name.is_some() || self.writer_2_ipi_name_num.is_some() ||
           self.writer_2_ipi_base_number.is_some() || self.submitter_work_num.is_some() {
            fields.push(format!("{:11}", self.iswc_of_original_work.as_deref().unwrap_or("")));
...
mod tests {
    ...
    fn test_ver_creation() {
        let ver = VerRecord::new(
            "00000001".to_string(),
            "00000001".to_string(),
            "Original Work Title".to_string(),
        );
        
        assert_eq!(ver.record_type, "VER");
        assert_eq!(ver.original_work_title, "Original Work Title");
    }
    
    #[test]
    fn test_ver_round_trip() {
        let original = VerRecord::new(
            "00000001".to_string(),
            "00000001".to_string(),
            "Original Work Title".to_string(),
        );
        ...
        let parsed = VerRecord::from_cwr_line(&line).unwrap();
