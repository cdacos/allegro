...
impl RecRecord {
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let release_date = line.get(19..27).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string());

        let constant = line.get(27..87).unwrap().to_string();

        let release_duration = line.get(87..93).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string());

        let constant2 = line.get(93..98).unwrap().to_string();

        let album_title = line.get(98..158).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string());

        let album_label = line.get(158..218).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string());

        let release_catalog_num = line.get(218..236).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string());

        let ean = line.get(236..249).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string());

        let isrc = line.get(249..261).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string());

        let recording_format = line.get(261..262).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string());

        let recording_technique = line.get(262..263).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string());

        // v2.1+ fields
        let media_type = if line.len() > 263 { line.get(263..266).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        // v2.2+ fields
        let recording_title = if line.len() > 266 { line.get(266..326).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let version_title = if line.len() > 326 { line.get(326..386).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let display_artist = if line.len() > 386 { line.get(386..446).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let record_label = if line.len() > 446 { line.get(446..506).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let isrc_validity = if line.len() > 506 { line.get(506..526).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let submitter_recording_identifier = if line.len() > 526 { line.get(526..540).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };
...
mod tests {
    ...
    fn test_rec_creation() {
        let rec = RecRecord::new("00000001".to_string(), "00000001".to_string());
    ...
    fn test_rec_round_trip() {
        let original = RecRecord::new("00000001".to_string(), "00000001".to_string());
