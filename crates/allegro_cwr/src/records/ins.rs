...
impl InsRecord {
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let number_of_voices = if line.len() > 19 {
            line.get(19..22)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let standard_instrumentation_type = if line.len() > 22 {
            line.get(22..25)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let instrumentation_description = if line.len() > 25 {
            line.get(25..75)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
    ...
    pub fn to_cwr_line(&self) -> String {
        ...
        if self.number_of_voices.is_some() || self.standard_instrumentation_type.is_some() ||
           self.instrumentation_description.is_some() {
