...
impl NprRecord {
    ...
    pub fn new(transaction_sequence_num: String, record_sequence_num: String) -> Self {
        Self { record_type: "NPR".to_string(), transaction_sequence_num, record_sequence_num, performing_artist_name: None, performing_artist_first_name: None, performing_artist_ipi_name_num: None, performing_artist_ipi_base_number: None, language_code: None, performance_language: None, performance_dialect: None }
    }
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let performing_artist_name = if line.len() > 19 { line.get(19..179).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let performing_artist_first_name = if line.len() > 179 { line.get(179..339).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let performing_artist_ipi_name_num = if line.len() > 339 { line.get(339..350).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let performing_artist_ipi_base_number = if line.len() > 350 { line.get(350..363).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let language_code = if line.len() > 363 { line.get(363..365).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let performance_language = if line.len() > 365 { line.get(365..367).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let performance_dialect = if line.len() > 367 { line.get(367..370).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        Ok(NprRecord { record_type, transaction_sequence_num, record_sequence_num, performing_artist_name, performing_artist_first_name, performing_artist_ipi_name_num, performing_artist_ipi_base_number, language_code, performance_language, performance_dialect })
    }
    ...
    pub fn to_cwr_line(&self) -> String {
        ...
        if self.performing_artist_name.is_some() || self.performing_artist_first_name.is_some() || self.performing_artist_ipi_name_num.is_some() || self.performing_artist_ipi_base_number.is_some() || self.language_code.is_some() || self.performance_language.is_some() || self.performance_dialect.is_some() {
...
mod tests {
    ...
    fn test_npr_creation() {
        let npr = NprRecord::new("00000001".to_string(), "00000001".to_string());
    ...
    fn test_npr_round_trip() {
        let original = NprRecord::new("00000001".to_string(), "00000001".to_string());
