...
impl PwrRecord {
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let publisher_ip_num = line.get(19..28)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let publisher_name = line.get(28..73)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let submitter_agreement_number = line.get(73..87)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let society_assigned_agreement_number = line.get(87..101)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let writer_ip_num = if line.len() > 101 {
            line.get(101..110)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let publisher_sequence_num = if line.len() > 110 {
            line.get(110..112)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
