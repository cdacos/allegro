...
impl HdrRecord {
    ...
    pub fn new(sender_type: String, sender_id: String, sender_name: String, edi_standard_version_number: String, creation_date: String, creation_time: String, transmission_date: String) -> Self {
        Self { record_type: "HDR".to_string(), sender_type, sender_id, sender_name, edi_standard_version_number, creation_date, creation_time, transmission_date, character_set: None, version: None, revision: None, software_package: None, software_package_version: None }
    }
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let character_set = if line.len() > 86 { line.get(86..101).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let version = if line.len() > 101 { line.get(101..104).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let revision = if line.len() > 104 { line.get(104..107).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let software_package = if line.len() > 107 { line.get(107..137).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let software_package_version = if line.len() > 137 { line.get(137..167).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        Ok(HdrRecord { record_type, sender_type, sender_id, sender_name, edi_standard_version_number, creation_date, creation_time, transmission_date, character_set, version, revision, software_package, software_package_version })
    }
    ...
    pub fn to_cwr_line(&self) -> String {
        ...
        if self.character_set.is_some() || self.version.is_some() || self.revision.is_some() || self.software_package.is_some() || self.software_package_version.is_some() {
            fields.push(format!("{:15}", self.character_set.as_deref().unwrap_or("")));
        }

        // Add optional v2.2+ fields
        if self.version.is_some() || self.revision.is_some() || self.software_package.is_some() || self.software_package_version.is_some() {
            fields.push(format!("{:3}", self.version.as_deref().unwrap_or("")));
        }

        if self.revision.is_some() || self.software_package.is_some() || self.software_package_version.is_some() {
...
mod tests {
    ...
    fn test_hdr_creation() {
        let hdr = HdrRecord::new("01".to_string(), "BMI".to_string(), "BMI".to_string(), "01.10".to_string(), "20050101".to_string(), "120000".to_string(), "20050101".to_string());
    ...
    fn test_hdr_round_trip() {
        let original = HdrRecord::new("01".to_string(), "BMI".to_string(), "BMI MUSIC".to_string(), "01.10".to_string(), "20050101".to_string(), "120000".to_string(), "20050101".to_string());
