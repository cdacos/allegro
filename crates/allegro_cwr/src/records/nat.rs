...
impl NatRecord {
    ...
    pub fn new(transaction_sequence_num: String, record_sequence_num: String, title: String, title_type: String) -> Self {
        Self { record_type: "NAT".to_string(), transaction_sequence_num, record_sequence_num, title, title_type, language_code: None }
    }
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let language_code = if line.len() > 661 { line.get(661..663).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        Ok(NatRecord { record_type, transaction_sequence_num, record_sequence_num, title, title_type, language_code })
    }

    /// Convert this record to a CWR format line
    pub fn to_cwr_line(&self) -> String {
        let mut fields = vec![format!("{:3}", self.record_type), format!("{:8}", self.transaction_sequence_num), format!("{:8}", self.record_sequence_num), format!("{:640}", self.title), format!("{:2}", self.title_type)];
...
mod tests {
    ...
    fn test_nat_creation() {
        let nat = NatRecord::new("00000001".to_string(), "00000001".to_string(), "Non-Roman Title".to_string(), "OT".to_string());
    ...
    fn test_nat_round_trip() {
        let original = NatRecord::new("00000001".to_string(), "00000001".to_string(), "Non-Roman Title".to_string(), "OT".to_string());
