...
impl NpnRecord {
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let language_code = if line.len() > 510 {
            line.get(510..512)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
    ...
    pub fn to_cwr_line(&self) -> String {
        let mut fields = vec![
            ...
            format!("{:480}", self.publisher_name)
        ];
...
mod tests {
    ...
    fn test_npn_creation() {
        let npn = NpnRecord::new(
            "00000001".to_string(),
            "00000001".to_string(),
            "01".to_string(),
            "123456789".to_string(),
            "Non-Roman Publisher Name".to_string(),
        );
