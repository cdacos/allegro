...
fn now_custom_validate(record: &mut NowRecord) -> Vec<CwrWarning<'static>> {
    ...
    if record.writer_name.as_str().trim().is_empty() {
        warnings.push(CwrWarning { field_name: "writer_name", field_title: "Writer name", source_str: std::borrow::Cow::Owned(record.writer_name.as_str().to_string()), level: WarningLevel::Critical, description: "Writer name cannot be empty".to_string() });
    }
    ...
    if let Some(ref position) = record.writer_position {
        if !position.as_str().trim().is_empty() && position.as_str().len() != 1 {
            warnings.push(CwrWarning { field_name: "writer_position", field_title: "Writer position (1 char, optional)", source_str: std::borrow::Cow::Owned(position.as_str().to_string()), level: WarningLevel::Warning, description: "Writer position must be exactly 1 character if specified".to_string() });
        }
    }
