...
impl NowRecord {
    ...
    pub fn new(transaction_sequence_num: String, record_sequence_num: String, writer_name: String, writer_first_name: String) -> Self {
        Self { record_type: "NOW".to_string(), transaction_sequence_num, record_sequence_num, writer_name, writer_first_name, language_code: None, writer_position: None }
    }
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let language_code = if line.len() > 339 { line.get(339..341).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let writer_position = if line.len() > 341 { line.get(341..342).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        Ok(NowRecord { record_type, transaction_sequence_num, record_sequence_num, writer_name, writer_first_name, language_code, writer_position })
    }

    /// Convert this record to a CWR format line
    pub fn to_cwr_line(&self) -> String {
        let mut fields = vec![format!("{:3}", self.record_type), format!("{:8}", self.transaction_sequence_num), format!("{:8}", self.record_sequence_num), format!("{:160}", self.writer_name), format!("{:160}", self.writer_first_name)];
...
mod tests {
    ...
    fn test_now_creation() {
        let now = NowRecord::new("00000001".to_string(), "00000001".to_string(), "Non-Roman Writer Name".to_string(), "Non-Roman First Name".to_string());
    ...
    fn test_now_round_trip() {
        let original = NowRecord::new("00000001".to_string(), "00000001".to_string(), "Non-Roman Writer Name".to_string(), "Non-Roman First Name".to_string());
