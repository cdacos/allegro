...
impl AckRecord {
    ...
    pub fn new(
        ...
        original_transaction_sequence_num: String,
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let creation_title = line.get(49..109)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let submitter_creation_num = line.get(109..129)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let recipient_creation_num = line.get(129..149)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
