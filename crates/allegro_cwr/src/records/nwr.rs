...
impl NwrRecord {
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let language_code = line.get(79..81)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
        ...
        let iswc = line.get(95..106)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let copyright_date = line.get(106..114)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let copyright_number = line.get(114..126)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
        ...
        let text_music_relationship = line.get(136..139)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let composite_type = line.get(139..142)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
        ...
        let excerpt_type = line.get(145..148)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let music_arrangement = line.get(148..151)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let lyric_adaptation = line.get(151..154)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let contact_name = line.get(154..184)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let contact_id = line.get(184..194)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let cwr_work_type = line.get(194..196)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let grand_rights_ind = line.get(196..197)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let composite_component_count = line.get(197..200)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let date_of_publication_of_printed_edition = line.get(200..208)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let exceptional_clause = line.get(208..209)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let opus_number = line.get(209..234)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let catalogue_number = line.get(234..259)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let priority_flag = if line.len() > 259 {
            line.get(259..260)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
...
mod tests {
    ...
    fn test_nwr_creation() {
        let nwr = NwrRecord::new(
            "NWR".to_string(),
            "00000001".to_string(),
            "00000001".to_string(),
            "Test Song".to_string(),
            "SW0000000001".to_string(),
            "SER".to_string(),
            "Y".to_string(),
            "ORI".to_string(),
        );
