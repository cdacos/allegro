...
impl SwtRecord {
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let pr_collection_share = line.get(28..33)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let mr_collection_share = line.get(33..38)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let sr_collection_share = line.get(38..43)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
        ...
        let shares_change = line.get(48..49)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let sequence_num = if line.len() > 49 {
            line.get(49..52)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
