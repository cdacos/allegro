...
impl SpuRecord {
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let agreement_type = if line.len() > 180 {
            line.get(180..182).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string())
        } else {
            None
        };
        
        let usa_license_ind = if line.len() > 182 {
            line.get(182..183).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string())
        } else {
            None
        };
