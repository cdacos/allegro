...
impl GrtRecord {
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let currency_indicator = if line.len() > 24 {
            line.get(24..27)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
        ...
        let total_monetary_value = if line.len() > 27 {
            line.get(27..37)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
    ...
    pub fn to_cwr_line(&self) -> String {
        vec![
            ...
            format!("{:3}", self.currency_indicator.as_deref().unwrap_or("")),
            format!("{:10}", self.total_monetary_value.as_deref().unwrap_or(""))
        ].join("")
    }
}
