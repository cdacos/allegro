...
impl AltRecord {
    ...
    pub fn new(transaction_sequence_num: String, record_sequence_num: String, alternate_title: String, title_type: String) -> Self {
        Self { record_type: "ALT".to_string(), transaction_sequence_num, record_sequence_num, alternate_title, title_type, language_code: None }
    }
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let language_code = if line.len() > 81 { line.get(81..83).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        Ok(AltRecord { record_type, transaction_sequence_num, record_sequence_num, alternate_title, title_type, language_code })
    }

    /// Convert this record to a CWR format line
    pub fn to_cwr_line(&self) -> String {
        let mut fields = vec![format!("{:3}", self.record_type), format!("{:8}", self.transaction_sequence_num), format!("{:8}", self.record_sequence_num), format!("{:60}", self.alternate_title), format!("{:2}", self.title_type)];
...
mod tests {
    ...
    fn test_alt_creation() {
        let alt = AltRecord::new("00000001".to_string(), "00000001".to_string(), "My Alternate Title".to_string(), "AT".to_string());
    ...
    fn test_alt_round_trip() {
        let original = AltRecord::new("00000001".to_string(), "00000001".to_string(), "My Alternate Title".to_string(), "AT".to_string());
