...
impl NwnRecord {
    ...
    pub fn new(transaction_sequence_num: String, record_sequence_num: String, writer_last_name: String) -> Self {
        Self { record_type: "NWN".to_string(), transaction_sequence_num, record_sequence_num, interested_party_num: None, writer_last_name, writer_first_name: None, language_code: None }
    }
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let writer_first_name = if line.len() > 188 { line.get(188..348).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        let language_code = if line.len() > 348 { line.get(348..350).map(|s| s.trim()).filter(|s| !s.is_empty()).map(|s| s.to_string()) } else { None };

        Ok(NwnRecord { record_type, transaction_sequence_num, record_sequence_num, interested_party_num, writer_last_name, writer_first_name, language_code })
    }

    /// Convert this record to a CWR format line
    pub fn to_cwr_line(&self) -> String {
        let mut fields = vec![format!("{:3}", self.record_type), format!("{:8}", self.transaction_sequence_num), format!("{:8}", self.record_sequence_num), format!("{:9}", self.interested_party_num.as_deref().unwrap_or("")), format!("{:160}", self.writer_last_name)];
...
mod tests {
    ...
    fn test_nwn_creation() {
        let nwn = NwnRecord::new("00000001".to_string(), "00000001".to_string(), "Non-Roman Writer Name".to_string());
    ...
    fn test_nwn_round_trip() {
        let original = NwnRecord::new("00000001".to_string(), "00000001".to_string(), "Non-Roman Writer Name".to_string());
