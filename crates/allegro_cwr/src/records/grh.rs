...
impl GrhRecord {
    ...
    pub fn new(
        ...
        Self {
            record_type: "GRH".to_string(),
            transaction_type,
            group_id,
            version_number,
            batch_request: None,
            submission_distribution_type: None,
        }
    }
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let batch_request = line.get(16..26)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let submission_distribution_type = line.get(26..28)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
