...
impl AgrRecord {
    ...
    pub fn from_cwr_line(line: &str) -> Result<Self, CwrParseError> {
        ...
        let international_standard_agreement_code = line.get(33..47)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
        ...
        let agreement_end_date = line.get(57..65)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let retention_end_date = line.get(65..73)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
        ...
        let prior_royalty_start_date = line.get(74..82)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
        ...
        let post_term_collection_end_date = line.get(83..91)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let date_of_signature_of_agreement = line.get(91..99)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
        ...
        let sales_manufacture_clause = line.get(104..105)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let shares_change = line.get(105..106)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let advance_given = line.get(106..107)
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .map(|s| s.to_string());
            
        let society_assigned_agreement_number = if line.len() > 107 {
            line.get(107..121)
                .map(|s| s.trim())
                .filter(|s| !s.is_empty())
                .map(|s| s.to_string())
...
mod tests {
    ...
    fn test_agr_creation() {
        let agr = AgrRecord::new(
            "00000001".to_string(),
            "00000001".to_string(),
            "AGREEMENT001".to_string(),
            "OS".to_string(),
            "20050101".to_string(),
            "Y".to_string(),
            "Y".to_string(),
