...
fn parse_cwr_line(line: &str, line_number: usize, context: &ParsingContext) -> Result<ParsedRecord, CwrParseError> {
    ...
    let record = match record_type {
        ...
        "SPU" | "OPU" => CwrRecord::Spu(SpuRecord::from_cwr_line_v2(line)?.record),
        "NPN" => CwrRecord::Npn(NpnRecord::from_cwr_line_v2(line)?.record),
        "SPT" | "OPT" => CwrRecord::Spt(SptRecord::from_cwr_line_v2(line)?.record),
        "SWR" | "OWR" => CwrRecord::Swr(SwrRecord::from_cwr_line_v2(line)?.record),
        "NWN" => CwrRecord::Nwn(NwnRecord::from_cwr_line_v2(line)?.record),
        "SWT" | "OWT" => CwrRecord::Swt(SwtRecord::from_cwr_line_v2(line)?.record),
        "PWR" => CwrRecord::Pwr(PwrRecord::from_cwr_line_v2(line)?.record),
        "ALT" => CwrRecord::Alt(AltRecord::from_cwr_line_v2(line)?.record),
        "NAT" => CwrRecord::Nat(NatRecord::from_cwr_line_v2(line)?.record),
        "EWT" => CwrRecord::Ewt(EwtRecord::from_cwr_line_v2(line)?.record),
        "VER" => CwrRecord::Ver(VerRecord::from_cwr_line_v2(line)?.record),
        "PER" => CwrRecord::Per(PerRecord::from_cwr_line_v2(line)?.record),
        "NPR" => CwrRecord::Npr(NprRecord::from_cwr_line_v2(line)?.record),
        "REC" => CwrRecord::Rec(RecRecord::from_cwr_line_v2(line)?.record),
        "ORN" => CwrRecord::Orn(OrnRecord::from_cwr_line_v2(line)?.record),
        "INS" => CwrRecord::Ins(InsRecord::from_cwr_line_v2(line)?.record),
        "IND" => CwrRecord::Ind(IndRecord::from_cwr_line_v2(line)?.record),
