...
use allegro_cwr::domain_types::{Boolean, Flag, LanguageCode, MusicalWorkDistributionCategory, Number, SocietyCode, TransactionStatus, VersionType};
    ...
    fn execute_insert(&self, statements: &mut PreparedStatements, tx: &rusqlite::Transaction, file_id: i64) -> Result<i64> {
        ...
        match self {
            ...
            allegro_cwr::CwrRegistry::Trl(trl) => {
                statements.trl_stmt.execute(params![file_id, "TRL", trl.group_count.to_sql_int(), trl.transaction_count.to_sql_int(), trl.record_count.to_sql_int()])?;
                Ok(tx.last_insert_rowid())
            }
            ...
            allegro_cwr::CwrRegistry::Ter(ter) => {
                statements.ter_stmt.execute(params![file_id, "TER", ter.transaction_sequence_num.as_str(), ter.record_sequence_num.as_str(), ter.inclusion_exclusion_indicator.to_sql_string(), ter.tis_numeric_code.to_sql_int()])?;
                Ok(tx.last_insert_rowid())
            }
            ...
            allegro_cwr::CwrRegistry::Npa(npa) => {
                statements.npa_stmt.execute(params![file_id, "NPA", npa.transaction_sequence_num.as_str(), npa.record_sequence_num.as_str(), npa.interested_party_num.as_deref(), npa.interested_party_name.as_str(), npa.interested_party_writer_first_name.as_str(), npa.language_code.as_deref()])?;
                Ok(tx.last_insert_rowid())
            }
            ...
            allegro_cwr::CwrRegistry::Npn(npn) => {
                statements.npn_stmt.execute(params![file_id, "NPN", npn.transaction_sequence_num.as_str(), npn.record_sequence_num.as_str(), npn.publisher_sequence_num.to_sql_int(), npn.interested_party_num.as_str(), npn.publisher_name.as_str(), npn.language_code.as_deref()])?;
                Ok(tx.last_insert_rowid())
            }
            ...
            allegro_cwr::CwrRegistry::Nwn(nwn) => {
                statements.nwn_stmt.execute(params![file_id, "NWN", nwn.transaction_sequence_num.as_str(), nwn.record_sequence_num.as_str(), nwn.interested_party_num.as_deref(), nwn.writer_last_name.as_str(), nwn.writer_first_name.as_deref(), nwn.language_code.as_deref()])?;
                Ok(tx.last_insert_rowid())
            }
            ...
            allegro_cwr::CwrRegistry::Alt(alt) => {
                statements.alt_stmt.execute(params![file_id, "ALT", alt.transaction_sequence_num.as_str(), alt.record_sequence_num.as_str(), alt.alternate_title.as_str(), alt.title_type.to_sql_string(), alt.language_code.as_deref()])?;
                Ok(tx.last_insert_rowid())
            }
            allegro_cwr::CwrRegistry::Nat(nat) => {
                statements.nat_stmt.execute(params![file_id, "NAT", nat.transaction_sequence_num.as_str(), nat.record_sequence_num.as_str(), nat.title.as_str(), nat.title_type.to_sql_string(), nat.language_code.as_deref()])?;
                Ok(tx.last_insert_rowid())
            }
            ...
            allegro_cwr::CwrRegistry::Net(net) => {
                statements.net_stmt.execute(params![file_id, "NET", net.transaction_sequence_num.as_str(), net.record_sequence_num.as_str(), net.title.as_str(), net.language_code.as_deref()])?;
                Ok(tx.last_insert_rowid())
            }
            allegro_cwr::CwrRegistry::Now(now) => {
                statements.now_stmt.execute(params![file_id, "NOW", now.transaction_sequence_num.as_str(), now.record_sequence_num.as_str(), now.writer_name.as_str(), now.writer_first_name.as_str(), now.language_code.as_deref(), now.writer_position.as_deref()])?;
                Ok(tx.last_insert_rowid())
            }
            allegro_cwr::CwrRegistry::Ari(ari) => {
                statements.ari_stmt.execute(params![file_id, "ARI", ari.transaction_sequence_num.as_str(), ari.record_sequence_num.as_str(), ari.society_num.to_sql_string(), ari.work_num.as_deref(), ari.type_of_right.to_sql_string(), ari.subject_code.as_ref().map(|x| x.as_str()), ari.note.as_deref()])?;
                Ok(tx.last_insert_rowid())
            }
            allegro_cwr::CwrRegistry::Xrf(xrf) => {
                statements.xrf_stmt.execute(params![file_id, "XRF", xrf.transaction_sequence_num.as_str(), xrf.record_sequence_num.as_str(), xrf.organisation_code.to_sql_string(), xrf.identifier.as_str(), xrf.identifier_type.to_sql_string(), xrf.validity.to_sql_string()])?;
                Ok(tx.last_insert_rowid())
            }
        }
    }
            ...
            match stmt.query_row(params![record_id], |row| {
                ...
                let hdr = allegro_cwr::records::HdrRecord {
                    ...
                    sender_type: {
                        ...
                        SenderType::from_sql_string(&row.get::<_, String>("sender_type")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    sender_id: {
                        use allegro_cwr::domain_types::SenderId;
                        SenderId::from_sql_string(&row.get::<_, String>("sender_id")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    sender_name: {
                        use allegro_cwr::domain_types::SenderName;
                        SenderName::from_sql_string(&row.get::<_, String>("sender_name")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    edi_standard_version_number: {
                        use allegro_cwr::domain_types::EdiStandardVersion;
                        EdiStandardVersion::from_sql_string(&row.get::<_, String>("edi_standard_version_number")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    creation_date: {
                    ...
                    creation_time: {
                        ...
                        Time::from_sql_string(&row.get::<_, String>("creation_time")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    transmission_date: {
                        use allegro_cwr::domain_types::Date;
                        Date::from_sql_string(&row.get::<_, String>("transmission_date")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    character_set: {
                        use allegro_cwr::domain_types::CharacterSet;
                        opt_string_to_domain::<CharacterSet>(row.get::<_, Option<String>>("character_set")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    version: {
            ...
            match stmt.query_row(params![record_id], |row| {
                let grh = allegro_cwr::records::GrhRecord {
                    ...
                    transaction_type: {
                        ...
                        TransactionType::from_sql_string(&row.get::<_, String>("transaction_type")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    group_id: {
                        use allegro_cwr::domain_types::GroupId;
                        GroupId::from_sql_string(&row.get::<_, String>("group_id")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    version_number: {
                    ...
                    submission_distribution_type: {
                        ...
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("submission_distribution_type")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                };
                Ok(grh)
            }) {
            ...
            match stmt.query_row(params![record_id], |row| {
                let grt = allegro_cwr::records::GrtRecord {
                    ...
                    transaction_count: {
                        ...
                        TransactionCount::from_sql_string(&row.get::<_, String>("transaction_count")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    record_count: {
                        use allegro_cwr::domain_types::RecordCount;
                        RecordCount::from_sql_string(&row.get::<_, String>("record_count")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    currency_indicator: {
            ...
            match stmt.query_row(params![record_id], |row| {
                let trl = allegro_cwr::records::TrlRecord {
                    ...
                    group_count: {
                        ...
                        GroupCount::from_sql_string(&row.get::<_, String>("group_count")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    transaction_count: {
                        use allegro_cwr::domain_types::TransactionCount;
                        TransactionCount::from_sql_string(&row.get::<_, String>("transaction_count")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
        ...
        "NWR" => {
            ...
            match stmt.query_row(params![record_id], |row| {
                let nwr = allegro_cwr::records::NwrRecord {
                    ...
                    language_code: opt_string_to_domain(row.get::<_, Option<String>>("language_code")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?,
                    ...
                    cwr_work_type: opt_string_to_domain(row.get::<_, Option<String>>("cwr_work_type")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?,
                    grand_rights_ind: {
                        use allegro_cwr::domain_types::Flag;
            ...
            match stmt.query_row(params![record_id], |row| {
                let agr = allegro_cwr::records::AgrRecord {
                    ...
                    agreement_start_date: {
                        use allegro_cwr::domain_types::Date;
                        Date::from_sql_string(&row.get::<_, String>("agreement_start_date")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    agreement_end_date: {
                        use allegro_cwr::domain_types::Date;
                        opt_string_to_domain::<Date>(row.get::<_, Option<String>>("agreement_end_date")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    retention_end_date: {
                        use allegro_cwr::domain_types::Date;
                        opt_string_to_domain::<Date>(row.get::<_, Option<String>>("retention_end_date")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    prior_royalty_status: {
                        use allegro_cwr::domain_types::PriorRoyaltyStatus;
                        PriorRoyaltyStatus::from_sql_string(&row.get::<_, String>("prior_royalty_status")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    prior_royalty_start_date: {
                        use allegro_cwr::domain_types::Date;
                        opt_string_to_domain::<Date>(row.get::<_, Option<String>>("prior_royalty_start_date")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    post_term_collection_status: {
                        use allegro_cwr::domain_types::PostTermCollectionStatus;
                        PostTermCollectionStatus::from_sql_string(&row.get::<_, String>("post_term_collection_status")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    post_term_collection_end_date: {
                        use allegro_cwr::domain_types::Date;
                        opt_string_to_domain::<Date>(row.get::<_, Option<String>>("post_term_collection_end_date")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    ...
                    sales_manufacture_clause: {
                        ...
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("sales_manufacture_clause")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
        ...
        "ACK" => {
            ...
            match stmt.query_row(params![record_id], |row| {
                let ack = allegro_cwr::records::AckRecord {
                    ...
                    original_group_id: {
                        use allegro_cwr::domain_types::GroupId;
                        GroupId::from_sql_string(&row.get::<_, String>("original_group_id")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    original_transaction_sequence_num: {
                        use allegro_cwr::domain_types::TransactionCount;
                        TransactionCount::from_sql_string(&row.get::<_, String>("original_transaction_sequence_num")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    original_transaction_type: {
                        use allegro_cwr::domain_types::TransactionType;
                        TransactionType::from_sql_string(&row.get::<_, String>("original_transaction_type")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    ...
                    processing_date: {
                        use allegro_cwr::domain_types::Date;
                        Date::from_sql_string(&row.get::<_, String>("processing_date")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    transaction_status: {
                        use crate::domain_conversions::CwrFromSqlString;
                        ...
                        TransactionStatus::from_sql_string(&row.get::<_, String>("transaction_status")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                };
                Ok(ack)
            }) {
            ...
            match stmt.query_row(params![record_id], |row| {
                let ipa = allegro_cwr::records::IpaRecord {
                    ...
                    interested_party_ipi_name_num: {
                        ...
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("interested_party_ipi_name_num")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    ipi_base_number: {
                        use crate::domain_conversions::opt_string_to_domain;
                        ...
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("ipi_base_number")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    ...
                    pr_affiliation_society: {
                        ...
                        opt_string_to_domain::<SocietyCode>(row.get::<_, Option<String>>("pr_affiliation_society")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    pr_share: {
                        use allegro_cwr::domain_types::OwnershipShare;
                        opt_string_to_numeric::<OwnershipShare>(row.get::<_, Option<String>>("pr_share")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    mr_affiliation_society: {
                        ...
                        opt_string_to_domain::<SocietyCode>(row.get::<_, Option<String>>("mr_affiliation_society")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    mr_share: {
                        use allegro_cwr::domain_types::OwnershipShare;
                        opt_string_to_numeric::<OwnershipShare>(row.get::<_, Option<String>>("mr_share")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    sr_affiliation_society: {
                        ...
                        opt_string_to_domain::<SocietyCode>(row.get::<_, Option<String>>("sr_affiliation_society")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    sr_share: {
                        use allegro_cwr::domain_types::OwnershipShare;
                        opt_string_to_numeric::<OwnershipShare>(row.get::<_, Option<String>>("sr_share")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                };
                Ok(ipa)
            }) {
            ...
            match stmt.query_row(params![record_id], |row| {
                let npa = allegro_cwr::records::NpaRecord {
                    ...
                    language_code: {
                        ...
                        opt_string_to_domain::<LanguageCode>(row.get::<_, Option<String>>("language_code")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                };
                Ok(npa)
            }) {
            ...
            match stmt.query_row(params![record_id], |row| {
                let spu = allegro_cwr::records::SpuRecord {
                    ...
                    publisher_type: {
                        ...
                        opt_string_to_domain::<PublisherType>(row.get::<_, Option<String>>("publisher_type")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    ...
                    publisher_ipi_name_num: {
                        ...
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("publisher_ipi_name_num")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    ...
                    pr_affiliation_society_num: {
                        ...
                        opt_string_to_domain::<SocietyCode>(row.get::<_, Option<String>>("pr_affiliation_society_num")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    pr_ownership_share: {
                        use allegro_cwr::domain_types::OwnershipShare;
                        opt_string_to_numeric::<OwnershipShare>(row.get::<_, Option<String>>("pr_ownership_share")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    mr_society: {
                        use crate::domain_conversions::opt_string_to_domain;
                        ...
                        opt_string_to_domain::<SocietyCode>(row.get::<_, Option<String>>("mr_society")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    mr_ownership_share: {
                        use allegro_cwr::domain_types::OwnershipShare;
                        opt_string_to_numeric::<OwnershipShare>(row.get::<_, Option<String>>("mr_ownership_share")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    sr_society: {
                        use crate::domain_conversions::opt_string_to_domain;
                        ...
                        opt_string_to_domain::<SocietyCode>(row.get::<_, Option<String>>("sr_society")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    sr_ownership_share: {
                        use allegro_cwr::domain_types::OwnershipShare;
                    ...
                    publisher_ipi_base_number: {
                        ...
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("publisher_ipi_base_number")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    international_standard_agreement_code: row.get::<_, Option<String>>("international_standard_agreement_code")?,
                    society_assigned_agreement_number: row.get::<_, Option<String>>("society_assigned_agreement_number")?,
                    agreement_type: {
        ...
        "SPT" => {
            ...
            match stmt.query_row(params![record_id], |row| {
                let spt = allegro_cwr::records::SptRecord {
                    ...
                    pr_collection_share: {
                        use allegro_cwr::domain_types::OwnershipShare;
                        opt_string_to_numeric::<OwnershipShare>(row.get::<_, Option<String>>("pr_collection_share")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    mr_collection_share: {
                        use allegro_cwr::domain_types::OwnershipShare;
                        opt_string_to_numeric::<OwnershipShare>(row.get::<_, Option<String>>("mr_collection_share")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    sr_collection_share: {
                        use allegro_cwr::domain_types::OwnershipShare;
                        opt_string_to_numeric::<OwnershipShare>(row.get::<_, Option<String>>("sr_collection_share")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    inclusion_exclusion_indicator: {
                        use allegro_cwr::domain_types::InclusionExclusionIndicator;
                        InclusionExclusionIndicator::from_sql_string(&row.get::<_, String>("inclusion_exclusion_indicator")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
            ...
            match stmt.query_row(params![record_id], |row| {
                let swr = allegro_cwr::records::SwrRecord {
                    ...
                    writer_ipi_name_num: {
                        ...
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("writer_ipi_name_num")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    pr_affiliation_society_num: {
                        use crate::domain_conversions::opt_string_to_domain;
                    ...
                    writer_ipi_base_number: {
                        ...
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("writer_ipi_base_number")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    personal_number: {
            ...
            match stmt.query_row(params![record_id], |row| {
                let nwn = allegro_cwr::records::NwnRecord {
                    ...
                    writer_last_name: {
                        ...
                        NonRomanAlphabet::from_sql_string(&row.get::<_, String>("writer_last_name")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    writer_first_name: {
                        use crate::domain_conversions::opt_string_to_domain;
                        ...
                        opt_string_to_domain::<NonRomanAlphabet>(row.get::<_, Option<String>>("writer_first_name")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    language_code: {
                        use crate::domain_conversions::opt_string_to_domain;
        ...
        "PWR" => {
            ...
            match stmt.query_row(params![record_id], |row| {
                let pwr = allegro_cwr::records::PwrRecord {
                    ...
                    publisher_sequence_num: {
                        use allegro_cwr::domain_types::PublisherSequenceNumber;
                        opt_string_to_numeric::<PublisherSequenceNumber>(row.get::<_, Option<String>>("publisher_sequence_num")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                };
                Ok(pwr)
            }) {
            ...
            match stmt.query_row(params![record_id], |row| {
                let alt = allegro_cwr::records::AltRecord {
                    ...
                    title_type: {
                        ...
                        TitleType::from_sql_string(&row.get::<_, String>("title_type")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    language_code: {
                        use crate::domain_conversions::opt_string_to_domain;
            ...
            match stmt.query_row(params![record_id], |row| {
                let ewt = allegro_cwr::records::EwtRecord {
                    ...
                    writer_1_ipi_name_num: {
                        ...
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("writer_1_ipi_name_num")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    ...
                    writer_2_ipi_name_num: {
                        ...
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("writer_2_ipi_name_num")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    writer_2_ipi_base_number: {
                        ...
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("writer_2_ipi_base_number")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    submitter_work_num: {
                        use crate::domain_conversions::opt_string_to_domain;
                        use allegro_cwr::domain_types::LookupPlaceholder;
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("submitter_work_num")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                };
                Ok(ewt)
            }) {
            ...
            match stmt.query_row(params![record_id], |row| {
                let ver = allegro_cwr::records::VerRecord {
                    ...
                    writer_1_ipi_base_number: {
                        use crate::domain_conversions::opt_string_to_domain;
                        use allegro_cwr::domain_types::LookupPlaceholder;
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("writer_1_ipi_base_number")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    writer_2_last_name: row.get::<_, Option<String>>("writer_2_last_name")?,
                    writer_2_first_name: row.get::<_, Option<String>>("writer_2_first_name")?,
                    writer_2_ipi_name_num: {
            ...
            match stmt.query_row(params![record_id], |row| {
                let per = allegro_cwr::records::PerRecord {
                    ...
                    performing_artist_ipi_name_num: {
                        ...
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("performing_artist_ipi_name_num")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    performing_artist_ipi_base_number: {
                        ...
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("performing_artist_ipi_base_number")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                };
                Ok(per)
            }) {
            ...
            match stmt.query_row(params![record_id], |row| {
                let npr = allegro_cwr::records::NprRecord {
                    ...
                    performance_language: {
                        ...
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("performance_language")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    performance_dialect: {
                        use crate::domain_conversions::opt_string_to_domain;
                        ...
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("performance_dialect")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                };
                Ok(npr)
            }) {
            ...
            match stmt.query_row(params![record_id], |row| {
                let rec = allegro_cwr::records::RecRecord {
                    ...
                    recording_format: {
                        ...
                        opt_string_to_domain::<RecordingFormat>(row.get::<_, Option<String>>("recording_format")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    recording_technique: {
                        use allegro_cwr::domain_types::RecordingTechnique;
                        opt_string_to_domain::<RecordingTechnique>(row.get::<_, Option<String>>("recording_technique")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    media_type: {
                        use crate::domain_conversions::opt_string_to_domain;
                        ...
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("media_type")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    ...
                    isrc_validity: {
                        ...
                        opt_string_to_domain::<IsrcValidityIndicator>(row.get::<_, Option<String>>("isrc_validity")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
            ...
            match stmt.query_row(params![record_id], |row| {
                let ins = allegro_cwr::records::InsRecord {
                    ...
                    standard_instrumentation_type: {
                        ...
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("standard_instrumentation_type")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
            ...
            match stmt.query_row(params![record_id], |row| {
                let ind = allegro_cwr::records::IndRecord {
                    ...
                    instrument_code: {
                        ...
                        InstrumentCode::from_sql_string(&row.get::<_, String>("instrument_code")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
            ...
            match stmt.query_row(params![record_id], |row| {
                let now = allegro_cwr::records::NowRecord {
                    ...
                    writer_name: {
                        use crate::domain_conversions::CwrFromSqlString;
                        use allegro_cwr::domain_types::NonRomanAlphabet;
                        NonRomanAlphabet::from_sql_string(&row.get::<_, String>("writer_name")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    writer_first_name: {
                        use crate::domain_conversions::CwrFromSqlString;
                        use allegro_cwr::domain_types::NonRomanAlphabet;
                        NonRomanAlphabet::from_sql_string(&row.get::<_, String>("writer_first_name")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    language_code: {
                        use crate::domain_conversions::opt_string_to_domain;
                    ...
                    writer_position: {
                        use crate::domain_conversions::opt_string_to_domain;
                        use allegro_cwr::domain_types::LookupPlaceholder;
                        opt_string_to_domain::<LookupPlaceholder>(row.get::<_, Option<String>>("writer_position")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                };
                Ok(now)
            }) {
            ...
            match stmt.query_row(params![record_id], |row| {
                let ari = allegro_cwr::records::AriRecord {
                    ...
                    type_of_right: {
                        ...
                        TypeOfRight::from_sql_string(&row.get::<_, String>("type_of_right")?).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
                    subject_code: {
                        use crate::domain_conversions::opt_string_to_domain;
                        use allegro_cwr::domain_types::SubjectCode;
                        opt_string_to_domain::<SubjectCode>(row.get::<_, Option<String>>("subject_code")?.as_deref()).map_err(|e| rusqlite::Error::InvalidColumnType(0, e, rusqlite::types::Type::Text))?
                    },
...
mod tests {
    ...
    fn test_sqlite_to_cwr_write_pattern() {
        ...
        let output_count = process_sqlite_to_cwr_with_version_and_output(db_file_path.to_str().unwrap(), file_id, Some(2.2), Some(output_file_path.to_str().unwrap())).unwrap();
