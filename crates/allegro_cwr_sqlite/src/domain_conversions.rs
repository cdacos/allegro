...
impl CwrFromSqlString for SenderType {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing SenderType: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed)
        }
    }
...
impl CwrFromSqlString for SenderId {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing SenderId: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed)
        }
    }
...
impl CwrFromSqlString for SenderName {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing SenderName: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed)
        }
    }
...
impl CwrFromSqlString for EdiStandardVersion {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing EdiStandardVersion: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed)
        }
    }
...
impl CwrFromSqlString for Date {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing Date: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed)
        }
    }
...
impl CwrFromSqlString for Time {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing Time: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed)
        }
    }
...
impl CwrFromSqlString for CharacterSet {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing CharacterSet: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
...
impl CwrFromSqlString for CwrVersion {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing CwrVersion: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed)
        }
    }
...
impl CwrFromSqlString for CwrRevision {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing CwrRevision: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed)
        }
    }
...
impl CwrFromSqlString for TransactionType {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing TransactionType: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed)
        }
    }
...
impl CwrFromSqlString for YesNo {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing YesNo: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed)
        }
    }
...
impl CwrFromSqlString for FlagYNU {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing FlagYNU: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed)
        }
    }
...
impl CwrFromSqlString for TitleType {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing TitleType: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed)
        }
    }
...
impl CwrFromSqlString for InclusionExclusionIndicator {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing InclusionExclusionIndicator: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed)
        }
    }
...
impl CwrFromSqlString for AgreementRoleCode {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing AgreementRoleCode: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed)
        }
    }
...
impl CwrFromSqlString for RecordingFormat {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing RecordingFormat: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed.unwrap_or_default())
        }
    }
...
impl CwrFromSqlString for RecordingTechnique {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing RecordingTechnique: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed.unwrap_or_default())
        }
    }
...
impl CwrFromSqlString for PublisherType {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing PublisherType: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed.unwrap_or_default())
        }
    }
...
impl CwrFromSqlString for PriorRoyaltyStatus {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing PriorRoyaltyStatus: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed)
        }
    }
...
impl CwrFromSqlString for PostTermCollectionStatus {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing PostTermCollectionStatus: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed)
        }
    }
...
impl CwrFromSqlString for CwrVersionNumber {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing CwrVersionNumber: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed)
        }
    }
...
impl CwrFromSqlString for CurrencyCode {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing CurrencyCode: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed)
        }
    }
...
impl CwrFromSqlString for Duration {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing Duration: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed.unwrap_or_default())
        }
    }
...
impl CwrFromSqlString for CompositeComponentCount {
    fn from_sql_string(value: &str) -> Result<Self, String> {
        ...
        if warnings.iter().any(|w| w.is_critical()) {
            Err(format!("Critical error parsing CompositeComponentCount: {}", warnings.iter().find(|w| w.is_critical()).unwrap().description))
        } else {
            Ok(parsed.unwrap_or_default())
        }
    }
...
impl CwrFromSqlInt for WorksCount {
    fn from_sql_int(value: i64) -> Result<Self, String> {
        if value < 0 || value > u32::MAX as i64 {
            Err(format!("WorksCount value {} is out of range", value))
        } else {
            Ok(WorksCount(value as u32))
        }
    }
}
